...
function postWebsites(title, description, image_url, category_id) {
  letbodyObj = {title, description, image_url, category_id}
  fetch(endPoint, {
    method: "POST",
  ...
  .then(response => response.json())
  .then(website => {
    const websiteData = website.data.mapletnewWebsite = new Website(websiteData, websiteData.attributes)
    document.querySelector('#website-container').innerHTML +=
    newWebsite.renderWebsite()
  })
}
